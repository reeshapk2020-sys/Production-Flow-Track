...
import { AlertCircle, Plus, Loader2, ShoppingCart, Pencil, ChevronDown, ChevronRight } from "lucide-react";
import {
  useListOrders, useCreateOrder, getListOrdersQueryKey,
  useUpdateOrder, useGetOrder
      ...
      <td colSpan={5} className="p-0">
        <div className="bg-background p-4 border-t">
          ...
          {batches.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-2">No batches linked to this order yet.</p>
          )}
        </div>
      </td>
    </tr>
...
export default function OrdersPage() {
  ...
  const { mutate: createOrd, isPending } = useCreateOrder({
    mutation: {
      onSuccess: () => {
  ...
  const { mutate: updateOrd, isPending: isUpdating } = useUpdateOrder({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListOrdersQueryKey() });
        setEditOpen(false);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Manage customer orders and view linked cutting batches</p>
          </div>
          {canCreate && (
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent>
                ...
                <form onSubmit={onSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">Order Number *</label>
                    <input name="orderNumber" required className="w-full border rounded-md px-3 py-2 text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">Customer Name *</label>
                    <input name="customerName" required className="w-full border rounded-md px-3 py-2 text-sm" />
                  </div>
                  <div>
                  ...
                  <Button type="submit" disabled={isPending} className="w-full">
                    {isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}Create Order
                  </Button>
                </form>
              </DialogContent>
            </Dialog>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-lg"><ShoppingCart className="h-5 w-5" /> Orders</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="flex justify-center py-12"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
            ) : !data?.length ? (
              <div className="text-center py-12 text-muted-foreground"><AlertCircle className="mx-auto h-10 w-10 mb-2 text-muted-foreground" /><p>No orders yet</p></div>
            ) : (
              <div className="rounded-md border overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {data.map((ord) => (
                      <>
                        <TableRow key={ord.id} className="cursor-pointer" onClick={() => setExpandedId(expandedId === ord.id ? null : ord.id)}>
                          <TableCell className="w-8">
                            {expandedId === ord.id ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                          </TableCell>
                          <TableCell className="font-mono text-xs font-semibold">{ord.orderNumber}</TableCell>
                          <TableCell>{ord.customerName}</TableCell>
                          <TableCell><StatusBadge status={ord.status} /></TableCell>
                          <TableCell className="text-xs text-muted-foreground">{ord.createdAt ? format(new Date(ord.createdAt), "dd MMM yyyy") : "-"}</TableCell>
                          {canEdit && (
                            <TableCell>
                              <Button variant="ghost" size="sm" onClick={(ev) => { ev.stopPropagation(); setEditTarget(ord); setEditOpen(true); }}>
                                <Pencil className="h-3.5 w-3.5" />
                              </Button>
                            </TableCell>
                          )}
                        </TableRow>
                        {expandedId === ord.id && <OrderDetailRow orderId={ord.id} />}
                      </>
                    ))}
                  </TableBody>
                </Table>
          ...
          <DialogContent>
            ...
            {editTarget && (
              <form onSubmit={onEditSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Order Number *</label>
                  <input name="orderNumber" required defaultValue={editTarget.orderNumber} className="w-full border rounded-md px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Customer Name *</label>
                  <input name="customerName" required defaultValue={editTarget.customerName} className="w-full border rounded-md px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Status</label>
                ...
                <Button type="submit" disabled={isUpdating} className="w-full">
                  {isUpdating ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : null}Update Order
                </Button>
              </form>
            )}
          </DialogContent>
