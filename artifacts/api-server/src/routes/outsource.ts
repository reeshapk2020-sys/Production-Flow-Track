...
router.post("/outsource/send", checkPermission("outsource", "create"), async (req, res) => {
  const { allocationId, quantitySent, outsourceCategory, vendorName, sendDate, remarks } = req.body;
  ...
  await logAudit(
    ...
    "outsource_transfers",
    String(transfer.id),
    `Sent ${quantitySent} pieces to outsource (${category}) for allocation #${allocation.allocationNumber}`
  );
