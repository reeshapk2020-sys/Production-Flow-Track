      ...
      <td colSpan={5} className="p-0">
        <div className="bg-slate-50 p-4 border-t">
          ...
          {batches.length > 0 ? (
            ...
            <p className="text-sm text-slate-500 text-center py-2">No batches linked to this order yet.</p>
          )}
        </div>
      </td>
    </tr>
    ...
    <AppLayout title="Orders">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-display font-bold text-slate-800">Orders</h2>
            <p className="text-sm text-slate-500 mt-1">Manage customer orders and view linked cutting batches</p>
          </div>
          {canCreate && (
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="text-center py-12 text-slate-500"><AlertCircle className="mx-auto h-10 w-10 mb-2 text-slate-300" /><p>No orders yet</p></div>
            ) : (
                    ...
                    {data.map((ord) => (
                      <>
                        <TableRow key={ord.id} className="cursor-pointer" onClick={() => setExpandedId(expandedId === ord.id ? null : ord.id)}>
                          ...
                          <TableCell className="text-xs text-slate-500">{ord.createdAt ? format(new Date(ord.createdAt), "dd MMM yyyy") : "-"}</TableCell>
                          {canEdit && (
                            <TableCell>
